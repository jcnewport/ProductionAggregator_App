/**
 * Diagnostic: dump the pdf-parse text for PDSWDX-DP-Anadarko-DAILY.pdf
 * so we can discover the daily column layout before wiring a parser.
 *
 * We expect Anadarko Daily to look similar to Anadarko Monthly (Format 1)
 * but with daily dates and possibly pressure columns. Confirm:
 *   - visual column order
 *   - API width (Anadarko Monthly was 14-digit)
 *   - row y-spacing for bucket size
 *   - detect signature: operator "Anadarko"/"Oxy"/"Occidental",
...
const FILE = 'PDSWDX-DP-Anadarko- DAILY.pdf';
