/**
 * Smoke test for the PDS Anadarko Daily PDF adapter (Format 5a).
 * Validates:
 *   - dispatcher routes to "PDS Anadarko Daily"
 *   - ≥1000 records from the 239-page sample (very conservative floor)
 *   - every record has wellName
 *   - every record has api10 (10 digits) and api14 (14 digits)
 *   - api14 starts with api10 (the true cross-parser invariant —
 *     Anadarko's 14-digit source makes it trivially true but we
 *     still assert it for consistency with Conoco/EOG)
 *   - every prodDate matches YYYY-MM-DD (NOT first-of-month — daily)
 *   - at least some prodDates are not day 1 (proves we kept daily
 *     granularity rather than collapsing to first-of-month)
 *   - oil/gas values look like daily (for each multi-day well,
 *     oilProd fluctuates rather than being strictly monotonic —
 *     cumulative would only grow)
 *   - no wellName equals a header label
 *   - choke, hoursDown, downtimeReason columns are populated on
 *     a reasonable percentage of rows
 */
...
const cases: {
  ...
  {
    file: 'PDSWDX-DP-Anadarko- DAILY.pdf',
    expectedFormat: 'PDS Anadarko Daily',
    minRecords: 1000,
  },
];
...
async function main() {
  ...
  for (const c of cases) {
    ...
    // oilProd must be DAILY, not cumulative.
    ...
    if (multiDay > 0 && fluctuated === 0) {
      console.log(
        `[FAIL] ${c.file}: ${multiDay} long-producing wells but NONE show oilProd fluctuation — may be cumulative`
      );
      fail++;
      continue;
    }
    ...
    const headerInName = outcome.records.filter((r) =>
      /^(Well Num|Well Name|Well ID|Prod Date|API|Oil Prod|Oil Sales|Gas Prod|Gas Sales|Water Prod|Water Inject|Tubing Pres\.?|Casing Pres\.?|Choke|Downtime|Downtime Reason)$/i.test(
        r.wellName.trim()
      )
    ).length;
    if (headerInName > 0) {
    ...
    const withHoursDown = outcome.records.filter((r) => typeof r.hoursDown === 'number').length;
    const withDtReason = outcome.records.filter((r) => r.downtimeReason && r.downtimeReason.length > 0).length;
    ...
    console.log(
      `       column coverage: tubingPres=${withTubing}/${n}, casingPres=${withCasing}/${n}, choke=${withChoke}/${n}, hoursDown=${withHoursDown}/${n}, dtReason=${withDtReason}/${n}, waterInj=${withWaterInj}/${n}, negativeOil=${negativeOil}`
    );
    console.log(
      `       daily-not-cumulative check: ${fluctuated}/${multiDay} long-producing wells show oilProd fluctuation`
    );

    const first = outcome.records[0];
    const last = outcome.records[outcome.records.length - 1];
    console.log(
      `       first: well="${first.wellName}" api10=${first.api10} api14=${first.api14} date=${first.prodDate} oil=${first.oilProd} gas=${first.gasProd} water=${first.waterProd} tub=${first.tubingPres} cas=${first.casingPres} choke=${first.choke} hrsDown=${first.hoursDown} reason="${first.downtimeReason}"`
    );
    console.log(
