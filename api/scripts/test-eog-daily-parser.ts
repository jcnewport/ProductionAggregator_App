/**
 * Smoke test for the PDS EOG Daily PDF adapter (Format 5b).
 * Same assertion structure as Anadarko/Conoco daily tests.
 */
...
const cases: {
  ...
  {
    file: 'PDSWDX-DP-EOG-DAILY.pdf',
    expectedFormat: 'PDS EOG Daily',
    minRecords: 500,
  },
];
...
async function main() {
  ...
  for (const c of cases) {
    ...
    if (multiDay > 0 && fluctuated === 0) {
      console.log(
        `[FAIL] ${c.file}: ${multiDay} long-producing wells but NONE show oilProd fluctuation`
      );
      fail++;
      continue;
    }
    ...
    const headerInName = outcome.records.filter((r) =>
      /^(Well Num|Well Name|Well ID|Prod Date|API|Oil Prod|Oil Sales|Gas Prod|Gas Sales|Water Prod|Water Inj|Tubing Pres\.?|Casing Pres\.?|Choke|Hours Down|Downtime Reason)$/i.test(
        r.wellName.trim()
      )
    ).length;
    if (headerInName > 0) {
    ...
    console.log(
      `       coverage: tubingPres=${withTubing}/${n}, casingPres=${withCasing}/${n}, choke=${withChoke}/${n}, hoursDown=${withHoursDown}/${n}, dtReason=${withDtReason}/${n}, waterInj=${withWaterInj}/${n}, negOil=${negativeOil}`
    );
    console.log(
      `       daily-not-cumulative: ${fluctuated}/${multiDay} long-producing wells show oilProd fluctuation`
    );

    const first = outcome.records[0];
    const last = outcome.records[outcome.records.length - 1];
    console.log(
      `       first: well="${first.wellName}" api10=${first.api10} api14=${first.api14} date=${first.prodDate} gas=${first.gasProd} oil=${first.oilProd} water=${first.waterProd} tub=${first.tubingPres} cas=${first.casingPres} choke=${first.choke}`
    );
    console.log(
